...
Head to [this page](https://app.baseten.co) to create an account with Baseten and generate an API key. Once you've done this, set the BASETEN\_API\_KEY environment variable:
...
```python Initialize with model slug icon="robot" theme={null}
...
```python Initialize with model URL icon="link" theme={null}
from langchain_baseten import ChatBaseten
...
```python Basic invocation icon="play" theme={null}
# Use the chat model
...
<CodeGroup>
  ```python Dictionary format icon="book" theme={null}
  messages = [
