...
* You can continue to use all vector store APIs (similarity\_search, retrievers, etc.) on the loaded\_store.
...
MMR (Maximal Marginal Relevance) balances two forces: relevance to the query and diversity among selected results, reducing near-duplicate answers. Control the trade-off with lambda\_mult (1.0 = all relevance, 0.0 = all diversity).
...
Turning the vector store into a retriever gives you a standard LangChain interface that chains (e.g., RetrievalQA) can call to fetch context. Under the hood it uses your chosen search type (similarity or mmr) and search\_kwargs.
