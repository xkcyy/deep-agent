...
import { Loader2, AlertCircle, FilePenLine, Check } from "lucide-react";

type Status = "pending" | "completed" | "error" | "interrupted";

interface FileWritePanelProps {
  status: Status;
...
function pickWriteResult(result: unknown): FileWriteResult {
  if (!result || typeof result === "string") return {};
  if (typeof result === "object") return result as FileWriteResult;
  return {};
}

function pickError(result: FileWriteResult): string | null {
  if (result.error) return String(result.error);
  return null;
}

export const FileWritePanel: React.FC<FileWritePanelProps> = ({
  status,
  args,
  result,
  rawResult,
}) => {
  const parsed = pickWriteResult(result);
